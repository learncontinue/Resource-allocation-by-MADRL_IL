...
def calcu_F_N(PMW, R, Loss, A_sat):
    ...
    for m in range(n_bs):
        for n in range(n_user):
            C[m, n] = 0.002 * Band * math.log(
                1 + (PMW[m, 0] * gmax * (10 ** (-Loss[m, n] / 10))) / ((10 ** ((-117) / 10)) + (10 ** ((-123) / 10))),
                2)
            # 第m个基站各天线分配功率相同
    ...
    for j in range(MAX_EP_STEPS):  # 一个回合200个时隙，一个时隙中进行用户关联和带宽分配
        ...
        for m in range(n_bs):
            ...
            if l <= 3:  # 如果范围内用户小于等于3个，那么基站给这几个服务。
                ...
                F_user = np.zeros((n_bs, n_user))
                c_ideal = np.zeros((n_bs, n_user))
                for mm in range(n_bs):
                    for n in range(n_user):
                        c_ideal[mm, n] = 0.002 * 500 * math.log(1 + (PMW[mm, 0] * 100 * (10 ** (-Loss_[mm, n] / 10))) / ((10 ** ((-117) / 10)) + (10 ** ((-123) / 10))),2)
                        # 第m个基站各天线分配功率相同
                        F_user[mm, n] = min(R[n], c_ideal[mm, n]) / R[n]
                #s_user = F_user[m,:]
                #将用户需求转化为用户满足率
        ...
        for m in range(M):
            ...
            while Pmaxnow >= Pbmin:
                PMW[m, :] = Pmaxnow / W
                PtolM[m] = Pmaxnow
                A1, Imax, A3, h = env.capacity_t_bs(choice_bs, PMW, A_sat)
                unity = Imax/Ith  # 通过计算基站对卫星用户的干扰来计算效用函数
                U[m] = unity
                if U[m] > 1:
...
np.save(r'E:\data\BS\else\ARA132', episode_reward / MAX_EP_STEPS)

...
class DeepQNetwork:#DQN网络，容量、n个行动、n个状态、学习率、奖励折扣因子、贪婪因子、目标替换间隔、批次大小和贪婪什么设置为None
    ...
    def load_weight_from_pkl1(self):  # 此函数用于加载训练好的权重
        checkpoint = torch.load(r'E:\data\LEO1\throu\DQN_net\save_para1.pth')
    ...
    def load_weight_from_pkl2(self):  # 此函数用于加载训练好的权重
        checkpoint = torch.load(r'E:\data\LEO1\throu\DQN_net\save_para2.pth')
        self.main_q_network.load_state_dict(checkpoint['main_q_network_state_dict'])
        self.target_q_network.load_state_dict(checkpoint['target_q_network_state_dict'])
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        self.cost_his = checkpoint['loss']

    def load_weight_from_pkl3(self):  # 此函数用于加载训练好的权重
        checkpoint = torch.load(r'E:\data\LEO1\throu\DQN_net\save_para3.pth')
        self.main_q_network.load_state_dict(checkpoint['main_q_network_state_dict'])
        self.target_q_network.load_state_dict(checkpoint['target_q_network_state_dict'])
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        self.cost_his = checkpoint['loss']

    def load_weight_from_pkl4(self):  # 此函数用于加载训练好的权重
        checkpoint = torch.load(r'E:\data\LEO1\throu\DQN_net\save_para4.pth')
        self.main_q_network.load_state_dict(checkpoint['main_q_network_state_dict'])
        self.target_q_network.load_state_dict(checkpoint['target_q_network_state_dict'])
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        self.cost_his = checkpoint['loss']

    def save_weight_to_pkl_net1(self):
        torch.save({
            # 'epoch': epochs,
            ...
            'loss': self.cost_his,
        }, r'E:\data\LEO1\throu\DQN_net\save_para1.pth')
        print(f"DQN_network参数保存成功")

    def save_weight_to_pkl_net2(self):
        torch.save({
            # 'epoch': epochs,
            'main_q_network_state_dict': self.main_q_network.state_dict(),
            'target_q_network_state_dict': self.target_q_network.state_dict(),
            'optimizer_state_dict': self.optimizer.state_dict(),
            'loss': self.cost_his,
        }, r'E:\data\LEO1\throu\DQN_net\save_para2.pth')
        print(f"DQN_network参数保存成功")

    def save_weight_to_pkl_net3(self):
        torch.save({
            # 'epoch': epochs,
            'main_q_network_state_dict': self.main_q_network.state_dict(),
            'target_q_network_state_dict': self.target_q_network.state_dict(),
            'optimizer_state_dict': self.optimizer.state_dict(),
            'loss': self.cost_his,
        }, r'E:\data\LEO1\throu\DQN_net\save_para3.pth')
        print(f"DQN_network参数保存成功")

    def save_weight_to_pkl_net4(self):
        torch.save({
            # 'epoch': epochs,
            'main_q_network_state_dict': self.main_q_network.state_dict(),
            'target_q_network_state_dict': self.target_q_network.state_dict(),
            'optimizer_state_dict': self.optimizer.state_dict(),
            'loss': self.cost_his,
        }, r'E:\data\LEO1\throu\DQN_net\save_para4.pth')
        print(f"DQN_network参数保存成功")
